...
import { selectUserProfile, selectUserRole, } from '../../../../redux/slices/auth.slice';
...
export default function ProfileIMG({ data }: any) {
  ...
  const {
    ...
    onSuccess: () => {
      Alert.success(t.imageUploaded);
      setBase64Img('');
    },
    onError: error => {
      Alert.warning(t.imageUploadError);
  ...
  return (
    <View style={styles.profileContainer}>
      ...
      <View style={styles.RecomendationContainer}>
        <Text style={styles.titleRecomendacion}>{t.recommendations}</Text>
