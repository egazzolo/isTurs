...
const ListOfTransferorsScreen: React.FC<HomeScreenProps> = ({navigation}) => {
  ...
  const {data: TransladistData} = useGetFetch<EmptyTransladista>('user/getMyTransfers');
  ...
  const renderTrasladoItem = ({item}: {item: User}) => (
    <View style={styles.trasladoItem}>
      <Image source={{uri: item.profile_img}} style={styles.image} />
      <View style={styles.infoContainer}>
        <Text style={styles.title}>{item.name}</Text>
        <Text style={styles.empresa}>
          {item.role === 'OPERATOR' ? t.operator : ''}
        </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        <Text style={styles.sectionHeader}>{t.listOfDrivers}</Text>
        <FlatList
